...
class Db_operations:
    ...
    def connect_db(self):
        try:
            connection = pymysql.Connect(host='localhost', port=3306, user='root', password='root', database='dimple_db', charset='utf8')
            print('DB connected')
            return connection
        except:
    ...
    def get_latest_row_id(self):
        ...
        cursor.close()
        self.disconnect_db(connection)
        return id[0] if id[0] is not None else 0
